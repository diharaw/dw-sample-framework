                                                                                                              ...
                                                                                                              m_position(position)
{
	...
	m_near_plane.normal = glm::vec3(0.0f);

	m_far_plane.point = position + m_forward * far;
	m_far_plane.normal = -m_forward;

	m_left_plane.point = position;
	float angle = PI / 2.0f + glm::radians(m_fov) / 2.0f + 0.1f;
	m_left_plane.normal = glm::vec3(glm::rotate(glm::mat4(1.0f), angle, m_up)*glm::vec4(m_forward, 1.0f));

	m_right_plane.point = position;
	m_right_plane.normal = glm::vec3(glm::rotate(glm::mat4(1.0f), -angle, m_up)*glm::vec4(m_forward, 1.0f));

	m_top_plane.point = position;
	m_top_plane.normal = glm::vec3(glm::rotate(glm::mat4(1.0f), angle, m_right)*glm::vec4(m_forward, 1.0f));

	m_bottom_plane.point = position;
	m_bottom_plane.normal = glm::vec3(glm::rotate(glm::mat4(1.0f), -angle, m_right)*glm::vec4(m_forward, 1.0f));
